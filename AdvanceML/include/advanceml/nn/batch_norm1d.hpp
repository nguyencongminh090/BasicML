...
/**
 * 1D batch normalization layer over a `(N, C)` input, normalizing per
 * feature: `y = gamma * (x - mean) / sqrt(var + eps) + beta`.
 *
 ...
 * handled entirely by the `batch_norm1d` op `forward()` calls -- this
 * class adds no `grad_fn` of its own.
 */
class BatchNorm1D : public Module {
public:
    /** Constructs a layer over `num_features` features; `momentum`/`eps` as in `batch_norm1d`. */
    explicit BatchNorm1D(size_t num_features, float momentum = 0.1f, float eps = 1e-5f);

...
class BatchNorm2D(Module):
    def __init__(self,
        ...
        self.gamma: Optional[Tensor] = Tensor(np.ones((1, num_features, 1, 1), dtype=np.float64), requires_grad=True) \
                                       if affine else None
        self.beta : Optional[Tensor] = Tensor(np.zeros((1, num_features, 1, 1), dtype=np.float64), requires_grad=True) \
                                       if affine else None

        self.running_mean: np.ndarray = np.zeros((1, num_features, 1, 1), dtype=np.float64)
        self.running_var : np.ndarray = np.ones((1, num_features, 1, 1), dtype=np.float64)
    ...
    def forward(self, X: ArrayLike) -> np.ndarray:
        ...
        if x.ndim != 4 or x.shape[1] != self.num_features:
            raise ValueError(f"expected input of shape (batch, {self.num_features}, H, W), got {x.shape}")

        axes = (0, 2, 3)
        if self.training:
            batch_mean = np.mean(x, axis=axes, keepdims=True)
            batch_var  = np.mean((x - batch_mean) ** 2, axis=axes, keepdims=True)
    ...
    def backward(self, grad_output: np.ndarray) -> np.ndarray:
        ...
        axes = (0, 2, 3)
        if self.affine and self.gamma is not None and self.beta is not None:
            self.gamma.grad += np.sum(grad_output * self.x_hat, axis=axes, keepdims=True)
            self.beta.grad  += np.sum(grad_output, axis=axes, keepdims=True)
            grad_x_hat = grad_output * self.gamma.data
        else:
            grad_x_hat = grad_output
        ...
        n         = grad_x_hat.shape[0] * grad_x_hat.shape[2] * grad_x_hat.shape[3]
        grad_var  = np.sum(grad_x_hat * self.x_centered, axis=axes, keepdims=True) * -0.5 * self.std_inv ** 3
        grad_mean = np.sum(grad_x_hat * -self.std_inv, axis=axes, keepdims=True) \
                    + grad_var * np.mean(-2.0 * self.x_centered, axis=axes, keepdims=True)
        return grad_x_hat * self.std_inv \
               + grad_var * 2.0 * self.x_centered / n \
               + grad_mean / n
